...
import pets from '../assets/const/pets';
const {height} = Dimensions.get('window');
const petCategories = [
  {name: 'MATERIALS', icon: 'palette-swatch'},
  {name: 'PAINTS', icon: 'format-paint'},
  {name: 'TOOLS', icon: 'knife'},
  {name: 'STEEL', icon: 'nail'},
];
...
const Card = ({pet, navigation}) => {
  return (
    <TouchableOpacity
      activeOpacity={0.8}
      onPress={() => navigation.navigate('OrderAddScreen', pet)}>
  ...
  return (
    <SafeAreaView style={{flex: 1, color: COLORS.white}}>
      <View style={style.header}>
        <Icon name="arrow-left" size={28} onPress={navigation.toggleDrawer} />
      ...
      {/* <TouchableOpacity
          ...
          onPress={() => navigation.navigate('ElectronicAdd')}
